...
VideoSource::VideoSource(int width, int height)
    : VideoSource(width, height, false) {}

VideoSource::VideoSource(int width, int height, bool pre_encoded)
    : width_(width), height_(height) {
  proto::FfiRequest req;
  auto* msg = req.mutable_new_video_source();
  msg->set_type(pre_encoded
                    ? proto::VideoSourceType::VIDEO_SOURCE_NATIVE_ENCODED
...
bool EncodedVideoSource::takeKeyFrameRequest() {
  ...
  if (!resp.has_take_video_key_frame_request()) {
    throw std::runtime_error(
        "FfiResponse missing take_video_key_frame_request");
  }
